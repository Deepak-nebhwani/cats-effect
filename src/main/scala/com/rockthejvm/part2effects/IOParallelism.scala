...
object IOParallelism extends IOApp.Simple {
  ...
  val goalInLifeParallel: IO.Par[String] =
    (parIO1: IO.Par[Int], parIO2: IO.Par[String]).mapN((num, string) =>
      s"my goal in life is $num and $string")
  ...
  val goalInLife_v3: IO[String] = (meaningOfLife.debug, favLang.debug).parMapN((num, string) =>
    s"my goal in life is $num and $string")
  ...
  val parallelWithFailure =
    (IO(Thread.sleep(1000)) >> meaningOfLife.debug, aFailure.debug).parMapN((num, string) =>
      s"$num $string")
  val parallelWithFailure_2 =
    (aFailure.debug, IO(Thread.sleep(1000)) >> meaningOfLife.debug).parMapN((string, num) =>
      s"$string $num ")
