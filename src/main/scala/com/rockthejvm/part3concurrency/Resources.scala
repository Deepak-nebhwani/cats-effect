...
object Resources extends IOApp.Simple {
  ...
  def bracketTestWithExceptionHandled = IO(new Connection("test/url/string")).bracket(conn =>
    for {
      con <- conn.open()
      result <- IO.raiseError[String](new RuntimeException("a proper fail"))
      after <- IO("after exception")
  ...
  val releaseResource: String => IO[Unit] =
    string => IO(s"finalizing the string: $string").debug.void
  ...
  def connFromConfResource(path: String) =
    Resource.make(IO("opening file").debug >> openFileScanner(path))(scanner =>
  ...
  def connFromConfResourceMoreClean_v2(path: String): Resource[IO, Connection] = for {
    scanner <- {
      ...
      val fileCloseIOFunction =
        (scanner: Scanner) => IO("closing file").debug >> IO(scanner.close())
