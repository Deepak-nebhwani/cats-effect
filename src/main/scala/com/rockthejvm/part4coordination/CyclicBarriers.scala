...
object CyclicBarriers extends IOApp.Simple {
  ...
  def createUser(id: Int, barrier: CBarrier): IO[Unit] = for {
    ...
    _ <-
      IO(s"[user $id] Just heard there's a new social network - signing up for the waitlist...").debug
  ...
  def openNetwork(): IO[Unit] = for {
    _ <-
      IO("[announcer] The Rock the JVM social network is up for registration! Launching when we have 10 users!").debug
    barrier <- CBarrier(10)
