...
object RacePractice extends IOApp.Simple {
  ...
  def timeOut[A](ioa: IO[A], duration: FiniteDuration): IO[Either[A, Unit]] = {
    ioa.onCancel(IO.raiseError(new RuntimeException(s"IO has been timeout after $duration"))).race(IO.sleep(duration))
  }
  ...
  def testLateIO = lateIO(IO.sleep(800 millis) >> IO("IO1"), IO.sleep(700 millis) >> IO("IO2")).debug
  ...
  def testRace_v2(duration: FiniteDuration) = race_v2( (IO.sleep(459 millis) >> IO.println("Hello")).onCancel(IO.raiseError(new RuntimeException(s"IO has been timeout after $duration"))),IO.sleep(duration))
